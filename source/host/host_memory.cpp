...
using PFN_UnmapViewOfFile2 = BOOL(WINAPI*)(_In_ HANDLE Process,
...
public:
    explicit Impl(size_t backing_size_, size_t virtual_size_)
        ...
        backing_handle = pfn_CreateFileMapping2(INVALID_HANDLE_VALUE,
                                                nullptr,
        ...
        backing_base = static_cast<u8*>(pfn_VirtualAlloc2(process,
                                                          nullptr,
                                                          backing_size,
        ...
        void* const ret = pfn_MapViewOfFile3(backing_handle,
                                             process,
                                             backing_base,
                                             0,
                                             backing_size,
        ...
        virtual_base = static_cast<u8*>(pfn_VirtualAlloc2(process,
                                                          nullptr,
                                                          virtual_size,
                                                          MEM_RESERVE | MEM_RESERVE_PLACEHOLDER,
                                                          PAGE_NOACCESS,
    ...
    void Free(size_t virtual_offset, size_t length) {}
    ...
    void Protect(size_t virtual_offset, size_t length, bool read, bool write) {
        ...
        while (it != end) {
            ...
            if (!VirtualProtect(virtual_base + offset, protect_length, new_flags, &old_flags)) {
                LOG_WARNING("Failed to change virtual memory protect rules");
            }
            ++it;
        }
    }
...
private:
    ...
    void Release() {
        if (!placeholders.empty()) {
            for (const auto& placeholder : placeholders) {
                if (!pfn_UnmapViewOfFile2(process,
                                          virtual_base + placeholder.lower(),
                                          MEM_PRESERVE_PLACEHOLDER)) {
                    LOG_WARNING("Failed to unmap virtual memory placeholder");
                }
            }
        ...
        if (virtual_base) {
            if (!VirtualFree(virtual_base, 0, MEM_RELEASE)) {
                LOG_WARNING("Failed to free virtual memory");
            }
        }
        if (backing_base) {
            if (!pfn_UnmapViewOfFile2(process, backing_base, MEM_PRESERVE_PLACEHOLDER)) {
                LOG_WARNING("Failed to unmap backing memory placeholder");
            }
            if (!VirtualFreeEx(process, backing_base, 0, MEM_RELEASE)) {
                LOG_WARNING("Failed to free backing memory");
            }
        }
        if (!CloseHandle(backing_handle)) {
            LOG_WARNING("Failed to free backing memory file handle");
        }
    }
    ...
    bool UnmapOnePlaceholder(size_t virtual_offset, size_t length) {
        ...
        if (!pfn_UnmapViewOfFile2(
                    process, virtual_base + placeholder_begin, MEM_PRESERVE_PLACEHOLDER)) {
            LOG_WARNING("Failed to unmap placeholder");
        }
    ...
    void MapView(size_t virtual_offset, size_t host_offset, size_t length) {
        if (!pfn_MapViewOfFile3(backing_handle,
                                process,
                                ...
                                nullptr,
                                0)) {
            LOG_WARNING("Failed to map placeholder");
        }
    }

    void Split(size_t virtual_offset, size_t length) {
        if (!VirtualFreeEx(process,
                           reinterpret_cast<LPVOID>(virtual_base + virtual_offset),
                           length,
                           MEM_RELEASE | MEM_PRESERVE_PLACEHOLDER)) {
            LOG_WARNING("Failed to split placeholder");
        }
    }
    ...
    void Coalesce(size_t virtual_offset, size_t length) {
        if (!VirtualFreeEx(process,
            ...
            LOG_WARNING("Failed to coalesce placeholders");
        }
    }
...
class HostMemory::Impl {
public:
    explicit Impl(size_t backing_size_) : backing_size{backing_size_}, virtual_size{} {
        bool good = false;
        SCOPE_EXIT({
            if (!good) {
                Release();
    ...
    explicit Impl(int fd_, size_t backing_size_, size_t virtual_size_)
            : fd{fd_}, backing_size{backing_size_}, virtual_size{virtual_size_} {
        bool good = false;
        SCOPE_EXIT({
            if (!good) {
                Release();
        ...
        virtual_base = static_cast<u8*>(mmap(nullptr,
                                             virtual_size,
                                             PROT_NONE,
    ...
    void Map(size_t virtual_offset, size_t host_offset, size_t length) {
        void* ret = mmap(virtual_base + virtual_offset,
                         length,
...
#ifdef __APPLE__
        madvise(backing_base + host_offset, length, MADV_DONTNEED);
#else
        madvise(backing_base + host_offset, length, MADV_REMOVE);
#endif
    }
...
HostMemory HostMemory::CreateForUser(int handle, size_t backing_size_, size_t virtual_size_) {
    return HostMemory{handle, backing_size_, virtual_size_};
}

HostMemory HostMemory::CreateForKern(size_t backing_size_) { return HostMemory{backing_size_}; }

HostMemory::HostMemory(size_t backing_size_) : backing_size(backing_size_) {
    try {
        // Try to allocate a fastmem arena.
        // The implementation will fail with std::bad_alloc on errors.
        impl = std::make_unique<HostMemory::Impl>(swift::AlignUp(backing_size, PageAlignment));
        backing_base = impl->backing_base;

    } catch (const std::bad_alloc&) {
        fallback_buffer = std::make_unique<swift::VirtualVector<u8>>(backing_size);
        backing_base = fallback_buffer->data();
        virtual_base = nullptr;
    }
}

HostMemory::HostMemory(int fd, size_t backing_size_, size_t virtual_size_)
    ...
    try {
        ...
        impl = std::make_unique<HostMemory::Impl>(
                fd,
                swift::AlignUp(backing_size, PageAlignment),
                swift::AlignUp(virtual_size, PageAlignment) + 3 * HugePageSize);
        backing_base = impl->backing_base;
...
void HostMemory::Free(size_t host_offset, size_t length) {
    ASSERT(host_offset % PageAlignment == 0);
    ASSERT(length % PageAlignment == 0);
    ASSERT(host_offset + length <= backing_size);
    if (length == 0 || !impl) {
    ...
    impl->Free(host_offset, length);
}

void HostMemory::Protect(size_t virtual_offset, size_t length, bool read, bool write, bool exec) {
    ASSERT(virtual_offset % PageAlignment == 0);
    ASSERT(length % PageAlignment == 0);
    ASSERT(virtual_offset + length <= virtual_size);
    if (length == 0 || !virtual_base || !impl) {
    ...
    impl->Protect(virtual_offset + virtual_base_offset, length, read, write, exec);
}
